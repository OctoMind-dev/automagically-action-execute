...
import { type GitHubContext, MAXIMUM_POLL_TIME_MILLISECONDS, sleep, TIME_BETWEEN_POLLS_MILLISECONDS } from "./utils";
...
export const executeTests = async ({
  ...
  await summary.addHeading("🐙 Octomind").addLink("View your Test Report", testReportUrl).write();

  if (blocking) {
    let currentStatus: TestReport["status"] | undefined = executeResponse.data.testReport.status;
    ...
    if (currentStatus !== "PASSED") {
      setFailed(`some test results failed, check your test report at ${testReportUrl} to find out more.`);
    }
  }
};
