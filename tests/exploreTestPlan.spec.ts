...
describe(exploreTestPlan.name, () => {
  ...
  it("calls explore API with correct parameters", async () => {
    ...
    expect(mockedClient.POST).toHaveBeenCalledWith("/apiKey/v3/test-plan/explore", {
      body: {
  ...
  it("creates summary on successful exploration", async () => {
    ...
    expect(core.summary.addHeading).toHaveBeenCalledWith("🐙 Octomind - Test Plan Exploration");
    expect(core.summary.addRaw).toHaveBeenCalledWith("Test plan exploration completed successfully");
  ...
  it("throws error when API returns no data", async () => {
    ...
    expect(core.setFailed).toHaveBeenCalledWith("test plan exploration did not return any data");
  });
  ...
  it("logs success message after completion", async () => {
    ...
    expect(core.info).toHaveBeenCalledWith("Test plan exploration completed successfully");
  });
});
